...
from app.FontCluster import FontCluster
...
class MergeFonts:
    ...
    def run(self):
        ...
        font_cluster = FontCluster(ttf_files)
        log_message(f"\nĐang phân cụm")
        clusters = font_cluster.cluster_fonts()

        # In kết quả nhóm
        for idx, cluster in enumerate(clusters, start=1):
            log_message(f"\n✅ Nhóm {idx}:")
            for font in cluster:
                log_message(f" - {font['name']}")

        # Gộp nhóm thành TTC hoặc copy nếu không thể gộp
        for i, font_paths in enumerate(clusters, start=1):
            if len(font_paths) == 1:
            ...
            else: self.merge_fonts_to_ttc(font_paths, self.longest_common_prefix(font_paths))
    ...
    def longest_common_prefix(self, names):
        if not names:
            return ""
        
        # Lấy chuỗi ngắn nhất để làm giới hạn
        min_length = min(len(name["name"]) for name in names)
        
        prefix = ""
        for i in range(min_length):
            char = names[0]["name"][i]  # Ký tự ở vị trí i của chuỗi đầu tiên
            flag = True
            for j in range(1, len(names)):
                if names[j]["name"][i] != char:
                    flag = False
                    break            
            if flag: 
                prefix += char
        ...
        return prefix.strip()  # Xóa khoảng trắng dư thừa (nếu có)